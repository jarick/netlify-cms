...
const getUrl = (link, direct) => `${ direct ? '/#' : '' }${ link }`;

export const getCollectionUrl = (collectionName, direct) => (
  getUrl(`/collections/${ collectionName }`, direct)
);

export const getNewEntryUrl = (collectionName, direct) => (
  getUrl(`/collections/${ collectionName }/new`, direct)
);

export const addParams = (urlString, params) => {
...
export const stripProtocol = (link) => {
  const protocolEndIndex = link.indexOf('//');

  return protocolEndIndex > -1 ? link.slice(protocolEndIndex + 2) : link;
};
...
export const sanitizeIRI = (str, { replacement = "" } = {}) => {
  if (!isString(str)) throw new Error('The input slug must be a string.');
  if (!isString(replacement)) throw new Error('options.replacement` must be a string.');

  // Check and make sure the replacement character is actually a safe char itself.
  if (!Array.from(replacement).every(validIRIChar)) {
...
export const sanitizeSlug = (str, { replacement = '-' } = {}) => {
  if (!isString(str)) throw new Error('The input slug must be a string.');
  if (!isString(replacement)) throw new Error('`options.replacement` must be a string.');
