...
const validateCollection = (configCollection) => {
  const collectionName = get(configCollection, 'name');
  if (
    !has(configCollection, 'folder')
    && !has(configCollection, 'files')
  ) {
    throw new Error(`Unknown collection type for collection "${ collectionName }". `
     + `Collections can be either Folder based or File based.`);
  }
  if (
    has(configCollection, 'format')
    && !supportedFormats.includes(get(configCollection, 'format'))
  ) {
    throw new Error(`Unknown collection format for collection "${ collectionName }". `
  ...
  if (
    ...
    throw new Error(`Please set a format for collection "${ collectionName }". `
      + `Supported formats are ${ supportedFormats.join(',') }`);
  }
  if (
    has(configCollection, 'frontmatter_delimiter')
    && !frontmatterFormats.includes(get(configCollection, 'format'))
  ) {
    // Cannot set custom delimiter without explicit and proper frontmatter format declaration
    throw new Error(`Please set a proper frontmatter format for collection "${ collectionName }" `
      + `to use a custom delimiter. Supported frontmatter formats are yaml-frontmatter,`
      + ` toml-frontmatter, and json-frontmatter.`
    );
  }
};
...
const selectors = {
  [FOLDER]: {
    entryExtension(collection) {
      return collection.get('extension')
    ...
    entryPath(collection, slug) {
      return `${ collection.get('folder').replace(/\/$/, '') }`
        + `/${ slug }.${ this.entryExtension(collection) }`;
    },
...
export const selectInferedField = (collection, fieldName) => {
  ...
  if (
    inferableField.fallbackToFirstField
    && mainTypeFields.size > 0
  ...
  if (inferableField.showError) {
    consoleError(
      ...
      `Netlify CMS tries to infer the entry ${ fieldName } automatically, but one couldn't `
        + `be found for entries of the collection “${ collection.get('name') }”. `
