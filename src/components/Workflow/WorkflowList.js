    ...
    if (!column) {
      return entries.entrySeq().map(([currColumn, currEntries]) => (
        <DropTarget
          ...
          key={currColumn}
          onDrop={this.handleChangeStatus(currColumn)}
        >
      ...
      <div>
        {
          entries.map((entry) => {
            ...
            const editLink = `collections/${ entry.getIn(['metaData', 'collection']) }` +
                ...
                {connect => connect(
                  <div>
                    <WorkflowCard
                      ...
                      editLink={editLink}
                      timestamp={timestamp}
                      onDelete={this.requestDelete(collection, slug, ownStatus)}
                      canPublish={canPublish}
                      onPublish={this.requestPublish(collection, slug, ownStatus)}
                    />
                  </div>
              )}
              </DragSource>
